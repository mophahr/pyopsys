...
class quantum_map:
	...
	def load_eigensystem( self, eva_file_name="eigenvalues", eve_file_name="eigenvectors" ):
	...
	def save_eigensystem( self, eva_file_name="eigenvalues", eve_file_name="eigenvectors", ):
	...
	def eigenvalues_sorted_by_modulus( self ):
		...
		if not self.sorted_exists:
			self.moduli = np.array([cmt.polar( eigenvalue )[0] for eigenvalue in eigenvalues])
...
class ternary_baker(quantum_map):
	...
	def __init__(self,M,reflectivity_center,reflectivity_left=1,reflectivity_right=1):
		...
		if not self.propagator_matrix_filled:
			def f( n, m ):
				if (n<self.M/3 and m<self.M/3):
					return mt.sqrt(self.R1)/mt.sqrt(self.M/3)*cmt.exp(-2*pi*1j/(self.M/3)*(n+1/2)*(m+1/2))
				elif (n>=self.M/3 and n<2/3*self.M and m>=self.M/3 and m<2/3*self.M):
					return mt.sqrt(self.R2)/mt.sqrt(self.M/3)*cmt.exp(-2*pi*1j/(self.M/3)*((n-self.M/3)+1/2)*((m-self.M/3)+1/2))
				elif (n>=self.M*2/3 and m>=self.M*2/3):
					return mt.sqrt(self.R3)/mt.sqrt(self.M/3)*cmt.exp(-2*pi*1j/(self.M/3)*((n-2*self.M/3)+1/2)*((m-2*self.M/3)+1/2))
				else:
					return 0
			self.propagator_matrix = np.array([[f(n,m) for n in range(self.M)] for m in range(self.M)])
			inverse_fourier_matrix = np.array([[1/mt.sqrt(self.M)*cmt.exp(2*pi*1j/self.M*(n+1/2)*(m+1/2)) for n in range(self.M)] for m in range(self.M)])
			self.propagator_matrix = inverse_fourier_matrix.dot(self.propagator_matrix)
...
class cat_map( quantum_map ):
	...
	def __init__(self,M,reflectivity_center,reflectivity_left=1,reflectivity_right=1, hole=[1/6,4/6]):
		quantum_map.__init__(self,M)
		if not M%2==0:
	...
	def propagator(self):
		if not self.propagator_matrix_filled:
			def f( n, m ):
				tmp = 1/mt.sqrt(M)*cmt.exp( 2*pi*1j/M* ( m*m -m*n + n*n)) 
				if n<self.R2_region_lower_bound:
					return mt.sqrt(self.R1)*tmp
				if (n>=self.R2_region_lower_bound and n<self.R2_region_upper_bound):
